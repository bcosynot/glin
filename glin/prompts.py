...
def worklog_entry_prompt(
    ...
    user = (
        ...
        "- DO break down into multiple bullets if more than one goal/objective is present.\n"
        ...
        "- Sort the content by date (ascending order) so that earliest date is on top and oldest date is at the bottom.\n"
        "- For each date D, first distill ONLY that date's content from your generated markdown into a concise bullet list "
        "  (use canonical keys in this order: **Goals**, **Work**, **PRs Merged**, **Metrics**, **Key Decision(s)**, **Impact**, **Learnings**; merge content into those bullets). Then immediately call the 'append_to_markdown' MCP tool with:\n"
        ...
        "Edge cases:\n"
        "- If a date has neither commits nor conversations, persist a single bullet: '- No recorded commits or conversations (planning/research/offline work possible).'.\n"
